...
// 여기서 useParam사용해도 되긴함 but 일관성을 위해 pno를 받도록함
const ModifyComponent = ({pno}) => {
    ...
    // 화면에서만 안보이게 하는 역할
    // 리액트에서는 데이터변경 -> 렌더링임을 명시하고 프론트내의 데이터를 변경해줘야함
    const deleteOldImages = (imageNames) => {
        const resultFileNames = product.uploadFileNames.filter(fileName => fileName !== imageNames)
